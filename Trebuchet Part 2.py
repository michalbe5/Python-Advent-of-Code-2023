...
def map_numbers(line):
    ...
    for idx, number in enumerate(words,1):
        if number in line:
            line = line.replace(number, number[0:1]+str(idx)+number[1:])
    return line

def convertsum(lines):
    numbers = []
    for line in lines.split():
        #print(line)
        line = map_numbers(line)
        #print(line)
        for letter in line:
            if letter.isnumeric():
                number = letter
...
print(convertsum(kox))


kox2 = '''fivepqxlpninevh2xxsnsgg63pbvdnqptmg
