...
for idx, line in enumerate(engine):
    for index, element in enumerate(line):
        if element.isnumeric():
            number += element
        if not element.isnumeric() or index==len(line)-1:
            if number:
                for x in range(index-len(number)-1,index+1):
                    if x<0:x=0
                    if x>=len(line):x=len(line)-1
                    if idx != 0:
                        if engine[idx-1][x] == '*':
                            if (idx-1, x) not in gears:
                                gears[(idx-1,x)] = [int(number)]
                            else:
                                gears[(idx-1,x)].append(int(number))
                    if idx < len(engine)-1:
                        if engine[idx+1][x] == '*':
                            if (idx+1, x) not in gears:
                                gears[(idx+1,x)] = [int(number)]
                                ...
                                gears[(idx+1,x)].append(int(number))
                    if engine[idx][x] == '*':
                            if (idx, x) not in gears:
                                gears[(idx,x)] = [int(number)]
                            else:
                                gears[(idx,x)].append(int(number))
                number = ''
gearratios = 0 
